...
with st.sidebar:
    ...
    st.markdown("### 🚦 Groq 限流保护")
    ai_call_daily_limit = st.number_input(
        "今日最多调用 Groq 次数",
        min_value=0, max_value=30, value=12, step=1,
        help="llama-3.3-70b-versatile 免费额度容易 429，建议先设 8-12。设为 0 可完全关闭 AI。"
    )
    ai_max_prompt_chars = st.number_input(
        "单次 AI 最大输入字数",
        min_value=1000, max_value=12000, value=5500, step=500,
        help="压缩 prompt，减少 token 消耗，避免当天额度被快速打满。"
    )
    if st.button("重置本地 AI 调用计数", width="stretch"):
        st.session_state["ai_call_date"] = datetime.now().strftime("%Y-%m-%d")
        st.session_state["ai_call_count"] = 0
        st.session_state["ai_blocked_today"] = False
        st.toast("已重置本地 AI 调用计数")
...
# ================= AI 计算核心：Groq 限流保护版 =================
def _reset_ai_counter_if_new_day():
    today = datetime.now().strftime("%Y-%m-%d")
    if st.session_state.get("ai_call_date") != today:
        st.session_state["ai_call_date"] = today
        st.session_state["ai_call_count"] = 0
        st.session_state["ai_blocked_today"] = False


def _local_ai_fallback(reason="AI 未运行"):
    return (
        f"⚠️ {reason}。已自动切换为【量化数据优先模式】：请先查看行情、资金热度、龙虎榜评分、支撑压力和风险提示。"
        "该提示不影响其他数据模块展示。"
    )


@st.cache_data(ttl=24 * 3600, show_spinner=False)
def _cached_groq_completion(model, prompt_hash, prompt, temperature):
    client = Groq(api_key=api_key, timeout=20)
...
def call_ai(prompt, model=None, temperature=0.3):
    _reset_ai_counter_if_new_day()

    if not api_key:
        return _local_ai_fallback("未配置 GROQ_API_KEY")
    if ai_call_daily_limit <= 0:
        return _local_ai_fallback("你已在侧边栏关闭 AI 调用")
    if st.session_state.get("ai_blocked_today", False):
        return _local_ai_fallback("Groq 今日已触发 429/额度限制")

    exec_model = model if model else selected_model
    prompt = str(prompt or "")[:int(ai_max_prompt_chars)]
    prompt_hash = hashlib.md5(f"{exec_model}|{temperature}|{prompt}".encode("utf-8")).hexdigest()

    cache_key = f"ai_cache_{prompt_hash}"
    if cache_key in st.session_state:
        return st.session_state[cache_key]

    used = int(st.session_state.get("ai_call_count", 0))
    if used >= int(ai_call_daily_limit):
        return _local_ai_fallback(f"已达到你设置的本地每日 AI 调用上限 {ai_call_daily_limit} 次")

    try:
        st.session_state["ai_call_count"] = used + 1
        content = _cached_groq_completion(exec_model, prompt_hash, prompt, temperature)
        st.session_state[cache_key] = content
        return content
    except Exception as e:
        err = str(e)
        if "429" in err or "rate_limit" in err.lower() or "rate limit" in err.lower():
            st.session_state["ai_blocked_today"] = True
            return _local_ai_fallback("Groq 返回 429 频率/额度限制")
        return _local_ai_fallback(f"AI 计算节点故障：{err[:180]}")
...
class LonghubangEngine:
    ...
    def run_comprehensive_analysis(self, date=None, lookback_days=10, ai_depth="标准", run_ai=True):
        ...
        if run_ai:
            agents_results = {}
            all_analyses = []
            # 限流保护：标准模式只调用 1 次 AI；深度模式最多 3 次 AI，避免一次运行就耗尽 Groq 免费额度。
            if ai_depth == "深度":
...
def render_high_end_news_terminal():
    ...
    with c5:
        use_ai_news = st.checkbox("生成 AI 情报报告", value=False)
...
with tab5:
    with st.container(border=True):
        ...
        with col_opt1:
            run_ai_lhb = st.checkbox("生成 AI 分析报告", value=False, help="关闭后只显示数据、评分和候选股票，速度最快。")
        with col_opt2:
