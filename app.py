...
# ================= v15 微创补丁：宏观看板极速缓存版 =================
def get_market_pulse_v15_fast():
    """宏观看板极速版：优先读最近真实缓存，再用短超时后台刷新；避免顶部看板拖慢整页。"""
    cache_path = _cache_file("market_pulse", "market_pulse_v15_fast.json")
    cached = _json_load(cache_path, max_age_seconds=180)
    ...
    pulse = {}
    def one_index(name, cfg):
        item, _ = _surgical_call_with_timeout(lambda: (_em_index_quote(cfg["em"], name) or _sina_index_quote(cfg["sina"], name)), timeout_sec=1.8, default=None)
        if item and safe_float(item.get("price"), 0) > 0:
            return name, item
        return name, {"price": 0.0, "pct": 0.0, "source": "指数实时源短时不可用", "status": "可继续使用下方功能"}
    threads = []
    q = _surgical_queue.Queue()
    for n, c in targets.items():
        t = _surgical_threading.Thread(target=lambda name=n, cfg=c: q.put(one_index(name, cfg)), daemon=True)
    ...
    while True:
        try:
            k, v = q.get_nowait(); pulse[k] = v
        except Exception:
            break
    for n in targets:
        pulse.setdefault(n, {"price": 0.0, "pct": 0.0, "source": "指数实时源短时不可用", "status": "可继续使用下方功能"})
    def get_cnh():
        ...
        cnh_res = fetch_json(cnh_url, timeout=1.5, retries=0)
        cnh_data = cnh_res.get("data") if isinstance(cnh_res, dict) else None
        if cnh_data:
            p = safe_float(cnh_data.get("f43"), 0.0)
            if p > 0:
                return {"price": p, "pct": safe_float(cnh_data.get("f170"), 0.0), "source": "东方财富外汇", "status": "真实接口"}
        return None
    cnh, _ = _surgical_call_with_timeout(get_cnh, timeout_sec=1.6, default=None)
    pulse["USD/CNH(离岸)"] = cnh or {"price": 0.0, "pct": 0.0, "source": "外汇实时源短时不可用", "status": "可继续使用下方功能"}
    if any(safe_float(v.get("price"), 0) > 0 for v in pulse.values() if isinstance(v, dict)):
        _json_save(cache_path, pulse)
    return pulse
# ================= v15 微创补丁：宏观看板极速缓存版结束 =================

st.markdown("### 🌍 宏观市场实时看板")
pulse_data = get_market_pulse_v15_fast()
if pulse_data:
...
# ================= v13 手术式修复层：只覆盖卡死根源，不改原 UI =================
# 目标：保留原页面结构和功能，只把资金热点/个股解析中会无限等待的接口改成“硬超时 + 快速真实链路 + 真实缓存”。
import queue as _surgical_queue
import threading as _surgical_threading


def _surgical_call_with_timeout(func, timeout_sec=5, default=None):
    """硬超时执行：避免 AKShare/网页接口底层阻塞导致 Streamlit spinner 永久转圈。"""
    q = _surgical_queue.Queue(maxsize=1)
    ...
    t = _surgical_threading.Thread(target=runner, daemon=True)
    t.start()
    t.join(float(timeout_sec))
    if t.is_alive():
        return default, f"timeout>{timeout_sec}s"
    try:
        ok, val = q.get_nowait()
    except Exception:
        return default, "no_result"
    if ok:
        return val, None
    return default, str(val)


def _em_clist_direct_fast(fields=None, fid="f62", pz=600):
    """东方财富 clist 快速直连：只请求一次，严格短超时，用于避免热点页长时间阻塞。"""
    fields = fields or "f1,f2,f3,f6,f8,f9,f12,f14,f20,f23,f62,f100,f184"
    base = "https://push2.eastmoney.com/api/qt/clist/get"
    params = {
        "pn": 1,
    ...
    headers = _eastmoney_headers(url, "https://quote.eastmoney.com/center/gridlist.html") if "_eastmoney_headers" in globals() else {"User-Agent": random.choice(USER_AGENTS)}
    res = SESSION.get(url, headers=headers, timeout=(2, 4), verify=False)
    if res.status_code != 200:
...
def _load_hot_blocks_fast_core():
    """资金热点核心真实链路：东财 f100/f62 聚合优先，必要时少量 AKShare 补充，整体由外层硬超时保护。"""
    records = []
    rows = []
    try:
        rows = _em_clist_direct_fast(fid="f62", pz=800)
    except Exception:
        rows = []
    if rows:
        try:
            records = _normalize_em_block_from_clist(rows) or []
        except Exception:
            records = []
    if records:
        return records
    ...
    try:
        ...
        if hasattr(ak, "stock_board_industry_name_em"):
            ak_try.append(("AKShare东财行业板块", lambda: ak.stock_board_industry_name_em()))
        if hasattr(ak, "stock_board_concept_name_em"):
            ak_try.append(("AKShare东财概念板块", lambda: ak.stock_board_concept_name_em()))
    except Exception:
        ak_try = []
    for source, loader in ak_try[:4]:
        df, err = _surgical_call_with_timeout(loader, timeout_sec=3, default=None)
        if df is None:
            continue
        try:
            recs = _normalize_block_records(df, source)
            if recs:
                records.extend(recs)
                if len(records) >= 12:
                    break
...
@st.cache_data(ttl=900, show_spinner=False)
def get_hot_blocks():
    """资金热点手术式修复版：固定时间内必须返回；不使用估算、不使用内置观察池。"""
    cache_path = _cache_file("blocks", f"hot_blocks_surgical_{datetime.now().strftime('%Y%m%d')}.json")
    records, err = _surgical_call_with_timeout(_load_hot_blocks_fast_core, timeout_sec=8, default=[])
    ...
    if globals().get("DEBUG_MODE", False):
        st.caption(f"资金热点真实接口超时/失败：{err or 'no data'}")
    return []


@st.cache_data(ttl=45, show_spinner=False)
def get_stock_quote(symbol):
    """个股解析手术式修复版：保留原字段，但每条行情链路硬超时，避免按钮永久加载。"""
    symbol = str(symbol).strip().zfill(6)
    if not re.fullmatch(r"\d{6}", symbol):
        return None
    cache_path = _cache_file("quote", f"quote_surgical_{symbol}.json")
    merged = {"symbol": symbol}
    fetchers = [
        ("东方财富单股实时", _quote_from_eastmoney_single_v7, 5),
        ("新浪实时", _quote_from_sina, 3),
        ("腾讯实时", _quote_from_tencent, 3),
        ("BaoStock T+1", _quote_from_baostock_t1, 4),
    ]
    for name, fetcher, timeout_sec in fetchers:
        data, err = _surgical_call_with_timeout(lambda f=fetcher: f(symbol), timeout_sec=timeout_sec, default=None)
        if data:
            merged = _merge_quote(merged, data)
            if _quote_quality(merged) >= 7 and safe_float(merged.get("price"), 0) > 0:
                break
        elif globals().get("DEBUG_MODE", False) and err:
            st.caption(f"{name} 跳过：{err}")
    ...
    if finalized:
        ...
        for field, label in [("market_cap", "总市值"), ("pe", "PE"), ("pb", "PB"), ("turnover", "换手率")]:
            if field in ["market_cap", "turnover"]:
                if safe_float(finalized.get(field), 0) <= 0:
                    missing.append(label)
            elif finalized.get(field) in (None, "", "-"):
                missing.append(label)
        finalized["fundamental_warning"] = "真实接口字段缺失：" + "、".join(missing) if missing else "真实接口字段较完整"
        _json_save(cache_path, finalized)
        return finalized

    cached = _json_load(cache_path, max_age_seconds=3600)
    if cached:
        cached["source"] = f"{cached.get('source','真实缓存')}｜缓存"
        return cached
    return None
...
@st.cache_data(ttl=900, show_spinner=False)
def get_kline(symbol, days=220):
    """日K手术式修复版：东财直连优先，BaoStock T+1 备用；AKShare仅短超时补充。"""
    symbol = str(symbol).strip().zfill(6)
    if not re.fullmatch(r"\d{6}", symbol):
        return None
    cache_path = _cache_file("kline", f"kline_surgical_{symbol}_{days}.json")

    loaders = [
        ("东方财富直连历史K", lambda: _fetch_em_daily_kline(symbol, days=days), 7),
        ("BaoStock T+1历史K", lambda: _fetch_baostock_daily_kline(symbol, days=days), 6),
        ("AKShare东财历史K", lambda: _normalize_daily_kline(ak.stock_zh_a_hist(symbol=symbol, period="daily", start_date=(datetime.now()-pd.Timedelta(days=days+260)).strftime("%Y%m%d"), end_date=datetime.now().strftime("%Y%m%d"), adjust="qfq"), days=days), 5),
    ]
    for source, loader, timeout_sec in loaders:
        df, err = _surgical_call_with_timeout(loader, timeout_sec=timeout_sec, default=None)
        if df is not None and not getattr(df, "empty", True):
            try:
                _json_save(cache_path, {"source": source, "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "data": df.assign(date=df["date"].dt.strftime("%Y-%m-%d")).to_dict("records")})
            except Exception:
                pass
            return df
        elif globals().get("DEBUG_MODE", False) and err:
            st.caption(f"{source} 跳过：{err}")

    cached = _json_load(cache_path, max_age_seconds=3 * 24 * 3600)
    if cached and isinstance(cached.get("data"), list):
        try:
            df = pd.DataFrame(cached["data"])
            df["date"] = pd.to_datetime(df["date"], errors="coerce")
...
# ================= v15 微创补丁：主力资金 / 龙虎榜精准修复 =================
def _em_clist_one_shot_v15(fields=None, fid="f62", pz=500, timeout_sec=4.5, cache_key="em_v15_one_shot"):
    """东财 clist 单次短请求：主力资金专用，不再 pz=5000 多轮降级，不再走 AKShare。"""
    fields = fields or "f1,f2,f3,f6,f8,f9,f12,f14,f20,f23,f62,f100,f184"
    def run():
        base = "https://push2.eastmoney.com/api/qt/clist/get"
        params = {"pn": 1, "pz": int(pz), "po": 1, "np": 1, "ut": "bd1d9ddb04089700cf9c27f6f7426281", "fltt": 2, "invt": 2, "fid": fid, "fs": "m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23", "fields": fields}
        data = fetch_eastmoney_native(base, params=params, timeout=4, retries=0, cache_key=f"{cache_key}_{datetime.now().strftime('%Y%m%d')}", cache_ttl=1800)
    ...
    rows, err = _surgical_call_with_timeout(run, timeout_sec=timeout_sec, default=[])
    return (rows, f"东方财富clist单次短请求成功 {len(rows)} 条") if rows else ([], f"东方财富clist单次短请求失败/超时：{err}")


def _mfs_fetch_eastmoney_money_rank_v15(self, pz=500):
    """主力资金 v15：只走东财 f62/f184 单次短请求 + 真实缓存；彻底绕开 AKShare/JQData 长阻塞。"""
    cache_path = _cache_file("main_force", f"em_money_rank_v15_{datetime.now().strftime('%Y%m%d')}.json")
    rows, msg = _em_clist_one_shot_v15(pz=min(int(pz or 500), 600), fid="f62", cache_key="main_force_v15")
    out = []
    for item in rows or []:
        code = self._normalize_code(item.get("f12", "")); name = str(item.get("f14", ""))
        if not re.fullmatch(r"\d{6}", code) or "ST" in name or "退" in name: continue
        out.append({"股票代码": code, "股票简称": name, "所属行业": item.get("f100") or self._infer_industry(name), "区间涨跌幅": safe_float(item.get("f3"), 0), "最新价": safe_float(item.get("f2"), 0), "主力净流入": safe_float(item.get("f62"), 0), "主力净占比": safe_float(item.get("f184"), 0), "成交额": safe_float(item.get("f6"), 0), "换手率": safe_float(item.get("f8"), 0), "总市值": _normalize_market_cap_yi(item.get("f20")), "市盈率": item.get("f9", "-"), "市净率": item.get("f23", "-"), "资金热度分": 0.0, "数据源": "东方财富真实资金字段f62/f184｜v15短请求"})
    if out:
        df = pd.DataFrame(out); _json_save(cache_path, {"time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "records": df.to_dict("records")}); return self._score_candidates(df), msg
    cached = _json_load(cache_path, max_age_seconds=8 * 3600)
    if cached and isinstance(cached.get("records"), list):
        df = pd.DataFrame(cached["records"])
        if not df.empty:
            df["数据源"] = f"真实缓存｜{cached.get('time','')}"; return self._score_candidates(df), f"实时主力资金短时不可用，使用 {cached.get('time','')} 真实缓存"
    return pd.DataFrame(), msg


def _mfs_get_main_force_v15(self, start_date=None, days_ago=None, min_market_cap=10.0, max_market_cap=5000.0):
    df, msg = self._fetch_eastmoney_money_rank(pz=500)
    return (True, df, msg) if df is not None and not df.empty else (False, pd.DataFrame(), f"真实主力资金暂不可用：{msg}。未使用估算数据，未使用内置观察池。")

try:
    MainForceStockSelector._fetch_eastmoney_money_rank = _mfs_fetch_eastmoney_money_rank_v15
    MainForceStockSelector.get_main_force_stocks = _mfs_get_main_force_v15
except Exception: pass


def render_main_force_tab():
    """主力资金 v15 UI：保留原模块风格，但去掉 JQData 状态与长链路，点击后最多等待固定时间。"""
    st.markdown("### 🐋 主力资金板块")
    st.info("本页只读取真实资金流字段：东方财富 f62/f184 为主；接口短时不可用则展示真实缓存或明确提示，不估算、不用内置股票池。")
    c_status1, c_status2, c_status3 = st.columns(3)
    c_status1.metric("核心数据源", "东方财富 f62/f184"); c_status2.metric("Tushare", "低频备用" if bool(ts_token) else "未启用"); c_status3.metric("JQData", "已降级关闭")
    st.markdown("---")
    col1, col2, col3 = st.columns(3)
    with col1:
        date_option = st.selectbox("监控时间区间", ["最近10天", "最近30天", "最近3个月"]); days_ago = 10 if date_option == "最近10天" else 30 if date_option == "最近30天" else 90
    with col2: final_n = st.slider("最终精选数量", 2, 10, 5)
    with col3: max_change = st.number_input("区间最大涨幅限制(%)", value=30.0, step=5.0, help="剔除已经大涨的高位股")
    use_ai = st.checkbox("生成AI解读", value=False, help="关闭后速度最快，只展示量化资金热度结果。")
    if st.button("🚀 启动主力追踪引擎", type="primary", width="stretch"):
        if not api_key and use_ai: st.warning("未配置 GROQ_API_KEY，将只展示快速量化结果。"); use_ai = False
        analyzer = MainForceAnalyzer()
        def run_analyzer(): return analyzer.run_full_analysis(start_date=None, days_ago=days_ago, final_n=final_n, max_range_change=max_change, min_market_cap=30.0, max_market_cap=5000.0, use_ai=use_ai)
        with st.spinner("正在快速提取真实主力资金字段并计算资金热度..."):
            result, err = _surgical_call_with_timeout(run_analyzer, timeout_sec=18 if use_ai else 9, default=None)
        if result is None: st.error(f"主力资金接口本次响应超时：{err}。请稍后重试；如已有真实缓存，系统会自动展示。"); return
        if result.get("success"):
            st.success(f"✅ 计算完成！已锁定 {len(result['final_recommendations'])} 只候选标的。数据源：{result.get('data_source_msg', analyzer.data_source_msg)}")
            if result.get("elapsed_fetch") is not None: st.caption(f"数据提取与初筛耗时：{result['elapsed_fetch']} 秒")
            if result.get("warning"): st.warning(result["warning"])
            if hasattr(analyzer, "raw_stocks") and analyzer.raw_stocks is not None and not analyzer.raw_stocks.empty:
                with st.expander("📋 查看快速候选池 Top 30", expanded=False):
                    show_cols = [c for c in ["股票代码", "股票简称", "所属行业", "资金热度分", "主力净流入", "主力净占比", "成交额", "区间涨跌幅", "换手率", "总市值", "市盈率", "数据源"] if c in analyzer.raw_stocks.columns]
                    st.dataframe(analyzer.raw_stocks[show_cols].head(30), width="stretch", hide_index=True)
            st.markdown("### ⭐ 首席精选标的池")
            for rec in result["final_recommendations"]:
                with st.expander(f"🏅 TOP {rec.get('rank', '-')} | {rec.get('name', '未知')} ({rec.get('symbol', '未知')})", expanded=True):
                    if rec.get("industry"): st.caption(f"所属方向：{rec.get('industry')}")
                    if rec.get("score") is not None: st.metric("资金热度分", f"{safe_float(rec.get('score')):.1f}")
                    st.markdown("**📌 核心逻辑：**")
                    for r in rec.get("reasons", []): st.write(f"- {r}")
                    st.markdown(f"**💰 建议仓位：** {rec.get('position', 'N/A')}"); st.markdown(f"**⚠️ 预警提示：** {rec.get('risks', 'N/A')}")
            st.markdown("---"); st.markdown("### 🤖 投研底稿")
            t1, t2 = st.tabs(["💰 资金流向透视", "📊 行业格局研判"])
            with t1: st.write(analyzer.fund_flow_analysis)
            with t2: st.write(analyzer.industry_analysis)
        else: st.error(f"❌ 运行失败: {result.get('error')}")


def _em_datacenter_lhb_v15(date: str):
    """龙虎榜 v15：东方财富 datacenter 分页抓取，补齐缺失，不只取第一页。"""
    date_dash = str(date)[:10]; cache_path = _cache_file("lhb", f"em_lhb_v15_{date_dash}.json")
    cached = _json_load(cache_path, max_age_seconds=12 * 3600)
    if cached and isinstance(cached.get("records"), list): return cached["records"]
    base = "https://datacenter-web.eastmoney.com/api/data/v1/get"; all_rows = []; total_pages = 1
    for page in range(1, 8):
        params = {"sortColumns": "TRADE_DATE,SECURITY_CODE", "sortTypes": "-1,1", "pageSize": 1000, "pageNumber": page, "reportName": "RPT_LHB_BOARDDETAIL", "columns": "ALL", "source": "WEB", "client": "WEB", "filter": f"(TRADE_DATE='{date_dash}')"}
        data, err = _surgical_call_with_timeout(lambda p=params: fetch_eastmoney_native(base, params=p, timeout=5, retries=0, cache_key=f"em_lhb_v15_{date_dash}_{page}", cache_ttl=12*3600, referer="https://data.eastmoney.com/stock/lhb.html"), timeout_sec=6, default=None)
        if not isinstance(data, dict): break
        result = data.get("result") or {}; rows = result.get("data", []) if isinstance(result, dict) else []
        if not rows and isinstance(data.get("data"), list): rows = data.get("data")
        if not isinstance(rows, list) or not rows: break
        all_rows.extend(rows)
        try: total_pages = max(total_pages, int(result.get("pages") or result.get("totalPage") or total_pages))
        except Exception: pass
        if page >= total_pages: break
    out = []
    for r in all_rows:
        code = str(r.get("SECURITY_CODE") or r.get("SECUCODE") or r.get("股票代码") or ""); m = re.search(r"(\d{6})", code); code = m.group(1) if m else code
        out.append({"gpdm": code, "gpmc": r.get("SECURITY_NAME_ABBR") or r.get("SECURITY_NAME") or r.get("股票简称") or "", "yzmc": r.get("OPERATEDEPT_NAME") or r.get("营业部名称") or r.get("OPERATEDEPT") or "龙虎榜席位", "yyb": r.get("OPERATEDEPT_NAME") or r.get("EXPLANATION") or r.get("上榜原因") or "", "sblx": r.get("EXPLANATION") or r.get("上榜原因") or r.get("BILLBOARD_TYPE") or "龙虎榜", "mrje": r.get("BUY") or r.get("BUY_AMT") or r.get("BUY_AMOUNT") or r.get("买入金额") or 0, "mcje": r.get("SELL") or r.get("SELL_AMT") or r.get("SELL_AMOUNT") or r.get("卖出金额") or 0, "jlrje": r.get("NET") or r.get("NET_BUY_AMT") or r.get("NET_AMOUNT") or r.get("净买额") or 0, "rq": str(r.get("TRADE_DATE") or date_dash)[:10], "gl": r.get("EXPLANATION") or r.get("上榜原因") or r.get("REASON") or ""})
    if out:
        seen = set(); dedup = []
        for x in out:
            key = (x.get("rq"), x.get("gpdm"), x.get("yzmc"), x.get("mrje"), x.get("mcje"))
            if key not in seen: seen.add(key); dedup.append(x)
        _json_save(cache_path, {"time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "records": dedup}); return dedup
    return []

try: _em_datacenter_lhb = _em_datacenter_lhb_v15
except Exception: pass
# ================= v15 微创补丁结束 =================



# ================= v15.1 微创补丁：龙虎榜多源合并补全 =================
def _v151_lhb_dedupe(records):
    seen, out = set(), []
    for r in records or []:
        if not isinstance(r, dict):
            continue
        key = (str(r.get('rq') or r.get('TRADE_DATE') or ''), str(r.get('gpdm') or r.get('股票代码') or ''), str(r.get('yzmc') or r.get('营业部名称') or ''), str(r.get('sblx') or r.get('上榜原因') or ''), str(r.get('mrje') or ''), str(r.get('mcje') or ''))
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
def _lhb_get_auto_v151(self, date, lookback_days=10):
    """同一交易日尽量合并东财数据中心、ws4、AKShare，避免只取第一个数据源导致缺明细。"""
    errors = []
    base_date = pd.to_datetime(date).date()
    for offset in range(0, lookback_days + 1):
        current_date = base_date - timedelta(days=offset)
        ...
        combined, sources = [], []
        calls = [
            ('东方财富数据中心分页RPT_LHB_BOARDDETAIL', lambda d=date_str: self.get_longhubang_data_eastmoney_datacenter(d), 5),
            ('ws4智瞰龙虎接口', lambda d=date_str: self.get_longhubang_data(d), 4),
            ('AKShare东方财富龙虎榜', lambda d=date_str: self.get_longhubang_data_akshare(d), 6),
        ]
        for source, loader, timeout_sec in calls:
            data, err = _surgical_call_with_timeout(loader, timeout_sec=timeout_sec, default=[])
            if data:
                combined.extend(data)
                sources.append(source)
            else:
                errors.append(f'{date_str} {source} 无数据/超时：{err or "空"}')
        combined = _v151_lhb_dedupe(combined)
        if combined:
            return {'success': True, 'requested_date': str(date), 'used_date': date_str, 'source': ' + '.join(sources), 'data': combined, 'errors': errors}
    return {'success': False, 'requested_date': str(date), 'used_date': None, 'source': '无可用源', 'data': [], 'errors': errors}

try:
    LonghubangDataFetcher.get_longhubang_data_auto = _lhb_get_auto_v151
except Exception:
    pass
# ================= v15.1 微创补丁结束 =================
